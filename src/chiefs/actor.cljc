(ns chiefs.actor
  "ChiefsActor — the ISCO-08 1113 traditional chief/village-head actor as
  a `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one administrative operation request
  ...
  The unconditional invariant: the ChiefsAdvisor can never
  directly commit a record or dispatch an administrative action the
  ChiefsGovernor refuses — every commit-record! call is gated
  behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [chiefs.advisor :as advisor]
            [chiefs.governor :as governor]
            [chiefs.store :as store]))

(defn build-graph
  "Build a compiled ChiefsActor graph. `store` implements
  `chiefs.store/Store`. `advisor` implements
  `chiefs.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:community-id (:community-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
